...
\begin{algorithm}
  ...
  \begin{algorithmic}
    ...
    \if {witnesses of $phi$ is not initialized}
      ...
      \state $wit_{1} \gets$ the first operand $op$ where $op \ne phi$
      \state $wit_{2} \gets$ the first operand $op$ after $wit_{1}$ where $op \ne phi$ \and $op \ne wit_{1}$
    \else
      \state \comment{update witnesses of $\phi$}
      \if {$wit_{1} = phi$ \or $wit_{1} = wit_{2}$}
        \state $wit_{1} \gets$ the first operand $op$ after $wit_{2}$ where $op \ne phi$ \and $op \ne wit_{2}$
      \endif
      \if {$wit_{2} = phi$ \or $wit_{1} = wit_{2}$}
        \state $wit_{2} \gets$ the first operand $op$ after $wit_{2}$ where $op \ne phi$ \and $op \ne wit_{2}$
      \endif
    \endif
    \if {$wit_{1}$ or $wit_{2}$ is empty}
      \return \FALSE
    \endif
    \endprocedure
  \end{algorithmic}
