---
layout: "post"
title: "LLVM 如何构造 SSA：Mem2Reg 算法的实现"
subtitle: "Mem2Reg"
author: "roife"
date: 2022-02-07
...
1. 在支配边界插入 $\varphi$ 指令（支配边界可以理解为“恰好支配不到的基本块”）
...
2. 插入 $\varphi$ 后，进行重命名
   ...
   - 如果遇到 `alloca`，直接删除
   - 如果遇到 `load`，读取 `IncomingVals[]`
   - 如果遇到 `store` 或者 $\varphi$，写入 `IncomingVals[]`
...
```swift
func mem2reg(Func fn) {
    allocas : [AllocaInst] = 需要消除的 alloca 指令
    alloca_defs : {AllocaInst : Vec<BasicBlocks>} = 每个 alloca 指令对应的 store 指令所在的基本块

    // 阶段 1：放置phi节点
    new_phis = {}
    for alloca in allocas {
        for bb in fn.bbs { bb.vis = false }
        worklist = alloca_defs[alloca]
        while !worklist.empty() {
            bb = worklist.pop()
            for df in bb.df {
                if df.vis { continue }
                new_phis.append((new PhiInst(df), alloca))
                worklist.append(df)
            }
        }
    }
    ...
    // 阶段 2：变量重命名
    for bb in fn.bbs { bb.vis = false }
    worklist = [(fn.bbs[0], allocas.map((_, UndefValue)))]
    while !worklist.empty {
        (bb, IncomingVals) = worklist.pop()
        if bb.vis { continue }

        bb.vis = true
        for inst in bb.inst {
            switch inst.ty {
                alloca  -> bb.remove(inst)
                load    -> {
                    L = load.addr
                    inst.replaceAllUseWith(IncomingVals[L])
                    bb.remove(inst)
                }
                store   -> {
                    L = store.addr
                    IncomingVals[L] = store.data
                    bb.remove(inst)
                }
                phi     -> {
                    IncomingVals[L] = phi
            ...
            for succ in bb.succs {
                worklist.append((succ, IncomingVals))
                for phi in succ.phi_insts {
                    if phi in new_phis.keys {
                        L = new_phis[phi]
                        phi.addEdge(IncomingVals[L], bb)
                    }
                }
            }
        }
